...
{
    public partial class frmPilihIntensif : Form
    {
        public string id, nama, nominal, jenis = "";

        void showData()
        {
            list.view("select * from tbl_intensif where status='AKTIF' and periode like '%" + cari.Text + "%' or nama like '%" + cari.Text + "%' or jenis like '%" + cari.Text + "%'");
        }
