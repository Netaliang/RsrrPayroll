...
{
    public partial class frmTunjangan : Form
    {
        ...
        void showData()
        {
            list.view("select * from tbl_tunjangan order by id asc");
        }
        ...
        private void frmTunjangan_Load(object sender, EventArgs e)
        {
            loadCombo();
        }
        ...
        {
            if(db.count("select * from tbl_rinciantunjangan where idTunjangan='" + idSelected + "'") == 0)
            {
                if (app.question("Hapus Tunjangan ?"))
                {
                    if (db.query("delete from tbl_tunjangan where id='" + idSelected + "'"))
                    {
                        app.info("Hapus Tunjangan Berhasil");
                        showData();
                        loadCombo();
                        periode.clear();
                        panel.clear();
        ...
        private void tambah_Click(object sender, EventArgs e)
        {
            ...
            {
                app.warn("Moohon Lengkapi Input Data Tunjangan");
            }
            else
            {
                if (app.question("Tambahkan Tunjangan?"))
                {
                    string period = bulan.SelectedItem.ToString() + tahun.SelectedItem.ToString();

                    string idTunjangan = id();
                    if (db.query("Insert into tbl_tunjangan values('" + idTunjangan + "','" + period + "','" + nama.Text + "','" + nominal.Text.ToAngka() + "','" + jenis.SelectedItem.ToString() + "','AKTIF')"))
                    {
                        //db.query("update tbl_gajipokok set statusgaji='NONAKTIF' where statuskaryawan='" + kat + "' and pendidikan='" + pend + "'");

                        app.info("Berhasil Menambahkan Tunjangan ");
                        periode.clear();
                        panel.clear();
                        showData();
                        loadCombo();
                ...
                {
                    app.info("Batal Menambahkan Tunjangan");
                    periode.clear();
                    panel.clear();
                    showData();
                    loadCombo();
        ...
        private void status(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (db.query("update tbl_tunjangan set status='" + com.Text.ToUpper() + "' where id='" + idSelected + "'"))
                    {
                        showData();
                        loadCombo();
